...
hypr_conf = Path(__file__).parent.parent / "data" / "nvidia.conf"
if hypr_conf.exists():
    with open(hypr_conf, "r") as conf:
        _HYPRLAND_NVIDIA_CONFIG = [line for line in conf.read().splitlines()]
hyprpipewire_conf = Path(__file__).parent.parent / "data" / "pipewire_hyprland.conf"
if hyprpipewire_conf.exists():
    with open(hyprpipewire_conf, "r") as conf:
        _HYPRLAND_PIPEWIRE_CONFIG = [line for line in conf.read().splitlines()]
...
def _write_hyprland_nvidia_config(username: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if hypr_conf.exists():
            ...
            content = "\n".join(line for line in _HYPRLAND_NVIDIA_CONFIG)
        lines_to_add = []
...
def _write_hyprland_pipewire_config(username: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if hypr_conf.exists():
            ...
            content = "\n".join(line for line in _HYPRLAND_PIPEWIRE_CONFIG)
